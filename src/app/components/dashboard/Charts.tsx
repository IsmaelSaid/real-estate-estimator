...
export function Charts({ data }: { data: Mutation[] }) {
  ...
  const houses = data.filter(
    (mutation) =>
      isHouse(mutation) &&
      isVente(mutation) &&
      mutation.valeur_fonciere < MAX_HOUSE_VALUE &&
      mutation.valeur_fonciere > MIN_HOUSE_VALUE,
  );
  const appartements = data.filter(
    (mutation) =>
      isAppartement(mutation) &&
      isVente(mutation) &&
      mutation.valeur_fonciere < MAX_APPARTEMENT_VALUE &&
      mutation.valeur_fonciere > MIN_APPARTEMENT_VALUE,
  );

  const housesSquareMeterValue = calculateSquareMeterValueByYear(houses);
  const appartementsSquareMeterValue =
    calculateSquareMeterValueByYear(appartements);
  const year = _.map(housesSquareMeterValue, "year");
  const pricesHouses = _.map(
    housesSquareMeterValue,
