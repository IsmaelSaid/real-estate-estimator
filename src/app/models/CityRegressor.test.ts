...
describe("City regressor ", () => {
  ...
  const target97418 = _.find(
    X_train,
    (mutation: Mutation) => mutation.code_commune === 97418,
  ) as Omit<Mutation, "valeur_fonciere">;
  const target97411 = _.chain(target97418)
    .clone()
    .set("code_commune", 97411)
    .value();
