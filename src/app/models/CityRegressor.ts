...
export class CityRegressor{
    ...
    public fit(X_train: Mutation[], Y_train: number[]): void {
        ...
        this.model = _.chain(this._X_train).groupBy((mutation: Mutation)=>mutation.code_commune).map((mutations: Mutation[],code_commune:string)=>{
            return {code_commune: parseInt(code_commune), prediction: _.meanBy(mutations,'valeur_fonciere')}
        }).value()
    }
}