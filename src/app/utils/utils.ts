...
export const calculateSquareMeterValueByYear = (mutations: Mutation[]) => {
  return _.chain(mutations)
    ...
    .map((value, year) => {
      const pricesPerSquareMeter = value.map((mutation: Mutation) =>
        calculateSquareMeterValue(mutation),
      );
...
export const countPropertyTypesByCity = (mutations: Mutation[]) => {
  return _.chain(mutations)
    ...
    .orderBy(
      (mutation) =>
        (mutation.count.Maison || 0) + (mutation.count.Appartement || 0),
      "desc",
    )
    .value();
};
  ...
  return _.chain(mutations)
    ...
    .map((value, year) => {
      const count = _.countBy(
        value,
        (mutation: Mutation) => mutation.type_local,
      );
...
export const isAppartement = (mutation: Mutation) =>
  mutation.type_local === "Appartement";
export const isVente = (mutation: Mutation) =>
  mutation.nature_mutation === "Vente";

export const extractTrainingData = (
  mutation: Mutation[],
  typeLocal: string,
  minSurface: number = 0,
  ...
  return _.chain(mutation)
    .uniqBy("id_mutation")
    .filter(
      (mutation: Mutation) => filterMutations(mutation) && isVente(mutation),
    )
...
const fetchAddresses = async (search: string) => {
  ...
  let preparedInput: { strings: string[]; numbers: number[] } = {
    strings: [],
    numbers: [],
  };
  _.chain(splitted)
  ...
  const url =
    "https://data.regionreunion.com/api/explore/v2.1/catalog/datasets/ban-lareunion/records?where=search(*,'RUE MARCEL PAGNOL SAINNTE MARIE') and numero in (3)&limit=20";
};
 ...
 * @param search
 */
export const SeparateNumbersFromStrings = (
  search: string,
): { strings: string[]; numbers: number[] } => {
  let preparedInput: { strings: string[]; numbers: number[] } = _.chain(search)
    .toUpper()
    .split(/[\s-]+/)
    .reduce(
      (acc, current) => {
